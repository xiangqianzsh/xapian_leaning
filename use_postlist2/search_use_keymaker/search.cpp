...
class MyKeyMaker : public Xapian::KeyMaker {
    std::string operator() (const Xapian::Document &doc) const {
        return Xapian::sortable_serialise(
                0.5 * Xapian::sortable_unserialise(doc.get_value(1)) + 
                0.7 * Xapian::sortable_unserialise(doc.get_value(2)) + 
                0.2 * Xapian::sortable_unserialise(doc.get_value(3)) + 
                0.4 * Xapian::sortable_unserialise(doc.get_value(4)) + 
                0.9 * Xapian::sortable_unserialise(doc.get_value(5)) 
                );
    }
};
