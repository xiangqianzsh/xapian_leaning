...
public:
    ...
    double get_value(const int idx) const {
        return slot_values[idx];
    }
    
    void set_value(const int idx, const std::string value) {
        slot_values[idx] = Xapian::sortable_unserialise(value);
    }
...
private:
    std::string operator() (const Xapian::Document &doc) const {
        ...
        return Xapian::sortable_serialise(
                0.5 * inMemoryDoc.get_value(1) + 
                0.7 * inMemoryDoc.get_value(2) + 
                0.2 * inMemoryDoc.get_value(3) + 
                0.4 * inMemoryDoc.get_value(4) + 
                0.9 * inMemoryDoc.get_value(5) 
                );
    }
